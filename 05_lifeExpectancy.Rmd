...
iso2.todiscard.manualPicked <- c("AG", "AW", "BB", "BN", "BS", "DJ", "FM", "GD", "GQ", "KG", "VC", "VI", "VC", "ST", "SB", "PF", "KI", "BZ", "LC", "PG")
...
main.regions <- regions.all[grep("all income levels", regions.all)]
...
le$tooltip <- as.character(le$country)


gpath <- ggplot(data = le) +   swi_theme(y_gridlines = F, base_size = 8, base_family = font) + 
  scale_x_continuous(breaks = seq(min(le$year), max(le$year), 6), expand = c(0, 0), name = "",
 ...
 coord_fixed(ratio = 0.9) + 
  ...
  scale_size_manual(values = c(strokeWidth, strokeWidth * 2.5,  strokeWidth * 5), guide=FALSE) +
  ...
  theme(
    ...
    axis.ticks.length=unit(2.7, "pt"),
    plot.margin = (plot.margin=unit(rep(0, 4), "mm")),
    plot.title=element_text(margin=margin(rep(0, 4)))
    )+
  labs(title = "", subtitle = "") +
  geom_text(data = subset(le, iso2c %in% 
      c("1W", countries.iso2, regions.iso2) & year == max(le$year)),
      aes(x = year, y = value, group = country, color = color, label = as.character(country)),
      size = 2, family = font, nudge_x = 0.3, nudge_y= 0.1, check_overlap = T, hjust = 0
	 )
gpath
...
igpath <- ggiraph(
  code = {print(gpath)},
  hover_css = "stroke-opacity:0.98;stroke-width:3px;",
  tooltip_opacity = 0.7,
  pointsize = 15,
  width = "100%",
  height_svg = 7.7,
  width_svg = 7,
  fontname_sans = "Geneva",
  fontname_serif = "Geneva"
)
igpath
...
save_html(
  tags$html(
    ...
    tags$body(    
      h2("Life expectancy"),
      div(class = "descr", HTML("In years, from 1960 until 2104. Each line is for a country or a region. Hover over line to highlight a series")),
